...
export default function AnalyticsCharts({ orders, completedOrders, dishes, categories }: AnalyticsChartsProps) {
  ...
  const inventoryData = useMemo(() => {
    ...
    // Calculate historical average (using all available data)
    const allTimeOrders = allOrders
    const allTimeDays = allTimeOrders.length > 0
      ? Math.max(1, Math.ceil((Date.now() - new Date(Math.min(...allTimeOrders.map(o => new Date(o.created_at).getTime()))).getTime()) / (24 * 60 * 60 * 1000)))
      : 1

    // Calculate per-dish inventory stats
    const dishInventory = dishes
      .filter(dish => inventoryCategories.includes(dish.category_id) && dish.is_active !== false)
      .map(dish => {
        // Current period
        ...
        // All time average
        const allTimeSales = allTimeOrders.flatMap(order =>
          (order.items || []).filter(item => item.dish_id === dish.id)
        )
        const allTimeQuantity = allTimeSales.reduce((sum, item) => sum + item.quantity, 0)
        const allTimeAvgPerDay = allTimeQuantity / allTimeDays

        // Calculate percentage change
        let percentageChange = 0
        if (allTimeAvgPerDay > 0) {
          percentageChange = ((periodAvgPerDay - allTimeAvgPerDay) / allTimeAvgPerDay) * 100
        }
        ...
        return {
          ...
          allTimeAvgPerDay: Math.round(allTimeAvgPerDay * 100) / 100,
  ...
  }, [allOrders, dishes, categories, inventoryPeriod, inventoryCategories, inventoryCustomStart, inventoryCustomEnd])

  return (
    <>
      {/* Tab Navigation */}
      <div className="flex items-center gap-4 mb-6">
        <Tabs value={activeAnalyticsTab} onValueChange={(v) => setActiveAnalyticsTab(v as 'overview' | 'inventory')} className="w-full">
          <TabsList className="grid w-full max-w-md grid-cols-2 h-12 p-1 bg-muted/50 rounded-xl">
            <TabsTrigger value="overview" className="rounded-lg data-[state=active]:bg-background data-[state=active]:shadow-sm h-10 font-semibold">
              <ChartLine size={18} className="mr-2" />
              Panoramica
            </TabsTrigger>
            <TabsTrigger value="inventory" className="rounded-lg data-[state=active]:bg-background data-[state=active]:shadow-sm h-10 font-semibold">
        ...
        <div className="space-y-6">
              ...
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground mt-1">
                    Monitora le porzioni vendute e confronta con la media storica
                  </p>
                </div>
            ...
            <CardContent className="pt-6">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                ...
                <div className="bg-muted/30 rounded-xl p-4 text-center">
                  ...
                  <p className="text-xs text-muted-foreground font-medium">Piatti Analizzati</p>
                </div>
                <div className="bg-muted/30 rounded-xl p-4 text-center">
                ...
                <div className="bg-muted/30 rounded-xl p-4 text-center">
                  <p className="text-2xl font-bold">{inventoryCategories.length}</p>
          ...
          <Card className="shadow-lg border-none overflow-hidden">
            <CardHeader className="bg-muted/10">
              <CardTitle className="text-lg">Dettaglio Porzioni per Piatto</CardTitle>
            </CardHeader>
                  ...
                  <table className="w-full">
                    <thead className="bg-muted/30">
                      <tr>
                        ...
                        <th className="text-center px-4 py-3 text-xs font-semibold uppercase tracking-wider text-muted-foreground">Venduti (Periodo)</th>
                        <th className="text-center px-4 py-3 text-xs font-semibold uppercase tracking-wider text-muted-foreground">Media/Giorno (Periodo)</th>
                        <th className="text-center px-4 py-3 text-xs font-semibold uppercase tracking-wider text-muted-foreground">Media Storica/Giorno</th>
                      ...
                      {inventoryData.dishes.map((dish, index) => (
                        <tr key={dish.id} className="hover:bg-muted/20 transition-colors">
                          <td className="px-4 py-3">
                            <div className="flex items-center gap-3">
                              <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-xs font-bold ${index < 3 ? 'bg-primary/10 text-primary' : 'bg-muted text-muted-foreground'
                                }`}>
                                {index + 1}
                              </div>
                          ...
                          <td className="px-4 py-3 text-center">
                            <span className="font-bold text-lg">{dish.periodQuantity}</span>
                          </td>
                          <td className="px-4 py-3 text-center">
                            <span className="font-semibold">{dish.periodAvgPerDay}</span>
                          </td>
                          <td className="px-4 py-3 text-center">
                            <span className="text-muted-foreground">{dish.allTimeAvgPerDay}</span>
                          </td>
                          <td className="px-4 py-3 text-center">
